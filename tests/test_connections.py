...
class BaseConnectionTest:
    ...
    def wait_for_connection_close(self, test_server, timeout=10):
        ...
        if test_string in laststdout:
            print("DEBUG: ✅ User connection already established in last output")
            return True
...
class TestConnections(BaseConnectionTest):
    ...
    def test_connection_scenarios(self, test_server, user_conn_type,
                                  engineer_conn_type):
...
class TestTCPMultipleConnections(BaseConnectionTest):
    ...
    def test_multiple_engineer_connections(self, test_server, tcp_count,
        ...
        self._test_multiple_mixed_engineers(test_server, tcp_count,
                                            udp_count, wss_count)

    @timeout_after(60)  # 1 minute timeout for failure test
    def test_nine_tcp_connections_should_fail(self, test_server):
        ...
        with pytest.raises((AssertionError, ConnectionError, OSError)):
            self._test_multiple_mixed_engineers(test_server, 9, 0, 0,
                                                expect_failure=True)
        ...
        try:
            ...
            for i, conn_type in enumerate(connection_plan):
                ...
                try:
                    ...
                    # Use shorter timeout for WSS connections
                    timeout = 15 if conn_type == 'wss' else 30
                    ...
                    # Add delay for WSS to prevent resource contention
                    if conn_type == 'wss':
                        print("    Waiting 1s to prevent SSL resource "
                              "contention...")
                        time.sleep(1.0)

                    # Add a small delay after WSS connections to prevent
                    # SSL handshake contention and child process conflicts
                    if conn_type == 'wss':
                        print("    WSS connection created - waiting 1.0s to "
